...
class CatalogController extends Controller
{
    ...
    function showProfileCatalog($id)
    {
        ...
        foreach ($services as $item) {
            ...
            array_push($servicesList, ['name' => $item->name, 'description' => $item->description, 'check' => $item->id ? true : false]);
        }
    ...
    {
        $profiles = Profile::where('active', 1)->where('section', 1)->limit(8)->get();
    ...
    {
        $profiles = Profile::where('active', 1)->where('section', 2)->limit(8)->get();
    ...
    {
        $profiles = Profile::where('active', 1)->where('section', 3)->limit(8)->get();
    ...
    {
        $profiles = Profile::where('active', 1)->where('section', 4)->limit(8)->get();
    ...
    {
        $profiles = Profile::where('active', 1)->where('section', 5)->limit(8)->get();
